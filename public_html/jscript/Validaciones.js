...
{  var pos1, pos2, bOk = true; 
   ...
   {  pos1 = sDir.indexOf('@', 0); 
     ...
     if (!bOk) 
       alert("Direcci�n de correo inv�lida"); 
   }
};
...
//    alert('1');
   if ( !textoObligatorio(f.dsc_fec_dsc.value) )
     ...
     if ( !esFecha (f.dsc_fec_dsc.value))
        Mensaje = Mensaje + "La Fecha de Descargo no es v\341lida  dd/mm/aaaa \n";
     else
...
//    alert('1.0');   
    if ( !textoObligatorio(f.dsc_doc_ges.value) && !textoObligatorio(f.dsc_doc_ges2.value))
        Mensaje = Mensaje + "Ingrese la gesti�n de Documento descargo.\n";
    else
      if (!IsNumero(f.dsc_doc_ges.value) && !IsNumero(f.dsc_doc_ges2.value))
        Mensaje = Mensaje + "La gesti�n de Documento descargo no es v�lida. \n";  
//    alert('1.1');   
    if ( !textoObligatorio(f.dsc_doc_adm.value) && !textoObligatorio(f.dsc_doc_adm2.value))
        Mensaje = Mensaje + "Ingrese la administraci�n de Documento descargo.\n";
    else
      if (!IsNumero(f.dsc_doc_adm.value) && !IsNumero(f.dsc_doc_adm2.value))
        Mensaje = Mensaje + "La administraci�n de Documento descargo no es v�lida. \n";  
//    alert('1.2');   
    if ( !textoObligatorio(f.dsc_doc_num.value) && !textoObligatorio(f.dsc_doc_num2.value))
        Mensaje = Mensaje + "Ingrese el n�mero de Documento descargo.\n";
    else
      if (!IsNumero(f.dsc_doc_num.value) && !IsNumero(f.dsc_doc_num2.value))
        Mensaje = Mensaje + "El n�mero de Documento descargo no es v�lido. \n";  
    }
*/
   ...
   if ( !textoObligatorio(f.dsc_des.value) )
      Mensaje = Mensaje + "Ingrese la Descripci\363n mercader�a.\n";

   if ( !textoObligatorio(f.dsc_adm.value) )
      Mensaje = Mensaje + "Ingrese la Administraci\363n.\n";
...
//    alert('1.3');         
   if ( !textoObligatorio(f.dsc_fec_inc.value) )
     ...
     if ( !esFecha (f.dsc_fec_inc.value))
        Mensaje = Mensaje + "La Fecha de Control no es v\341lida  dd/mm/aaaa \n";
     else
...
//    alert('1.4');         
    if (textoObligatorio(f.dsc_cnt.value) && !IsNumero(f.dsc_cnt.value))
       Mensaje = Mensaje + "La Cantidad mercader\355a no es v\363lida. \n"; 
  ...
  var hoy = dia+'/'+mes+'/'+anio;
   ...
   if ( !textoObligatorio(f.dsc_fec.value) )
     ...
     if ( !esFecha (f.dsc_fec.value))
        Mensaje = Mensaje + "La Fecha no es v\341lida  dd/mm/aaa \n";
   else
...
{  f = document.forms[1];
   ...
   if(!textoObligatorio(f.val_fec_dsc1.value) && !textoObligatorio(f.val_fec_dsc2.value)
      ...
      Mensaje = Mensaje + "Ingrese alg\372n par\341metro de b\372squeda.\n";
   else
      if ( textoObligatorio(f.val_fec_dsc1.value) || textoObligatorio(f.val_fec_dsc2.value))
          if ( !esFecha (f.val_fec_dsc1.value) || !esFecha (f.val_fec_dsc2.value))
                Mensaje = Mensaje + "La Fecha no es v\341lida o falta alguna de las Fechas.\n"
...
{  f = document.forms[1];
   ...
   else
   {
    ...
    if ( !textoObligatorio(f.key_year.value) && !textoObligatorio(f.key_year2.value))
        Mensaje = Mensaje + "Ingrese la gesti\363n de Documento descargo.\n";
       else
         if (!IsNumero(f.key_year.value) && !IsNumero(f.key_year2.value))
            Mensaje = Mensaje + "La gesti\363n de Documento descargo no es v\341lida. \n";  
         else
           {
    ...
    if ( !textoObligatorio(f.key_cuo.value) && !textoObligatorio(f.key_cuo2.value))
        Mensaje = Mensaje + "Ingrese la administraci\363n de Documento descargo.\n";
    else
      if (!IsNumero(f.key_cuo.value) && !IsNumero(f.key_cuo2.value))
        Mensaje = Mensaje + "La administraci\363n de Documento descargo no es v\341lida. \n";  
    if ( !textoObligatorio(f.reg_nber.value) && !textoObligatorio(f.reg_nber2.value))
        Mensaje = Mensaje + "Ingrese el n\372mero de Documento descargo.\n";
    else
      if (!IsNumero(f.reg_nber.value) && !IsNumero(f.reg_nber2.value))
        Mensaje = Mensaje + "El n\372mero de Documento descargo no es v\341lido. \n";  
    }
   }
     if (Mensaje == "")
      return true;
